...
/* Created on:     30/08/2015 23:28:47                          */
/*==============================================================*/
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('SCPM_PERSONALES') and o.name = 'FK_SCPM_PER_RELATIONS_SCPM_CON')
alter table SCPM_PERSONALES
   drop constraint FK_SCPM_PER_RELATIONS_SCPM_CON
go
...
if exists (select 1
           ...
           where  id    = object_id('SCPM_CONYUGES')
            ...
            and   indid > 0
            and   indid < 255)
   drop index SCPM_CONYUGES.RELATIONSHIP_9_FK
go
...
if exists (select 1
           ...
           where  id    = object_id('SCPM_PERSONALES')
            ...
            and   indid > 0
            and   indid < 255)
   drop index SCPM_PERSONALES.RELATIONSHIP_8_FK
go
...
create table SCPM_CONYUGES (
   ...
   constraint PK_SCPM_CONYUGES primary key nonclustered (CON_ID)
)
go
...
/*==============================================================*/
create index RELATIONSHIP_9_FK on SCPM_CONYUGES (
PER_ID ASC
)
go
...
select @CurrentUser = user_name()
execute sp_dropextendedproperty 'MS_Description', 
   'user', @CurrentUser, 'table', 'SCPM_PERSONALES', 'column', 'CON_ID'
...
/*==============================================================*/
create index RELATIONSHIP_8_FK on SCPM_PERSONALES (
CON_ID ASC
)
go
...
alter table SCPM_PERSONALES
   add constraint FK_SCPM_PER_RELATIONS_SCPM_CON foreign key (CON_ID)
      references SCPM_CONYUGES (CON_ID)
go
